...
####################################################
index = "Deaths"
dfplot = df_incidence %>% filter(measure_name=="Deaths")
...
ggplot(df_asr) +
  ...
  labs(title ="") +
  scale_fill_manual(name="Age specific rate",
                    values = (c(brewer.pal(n = 9, name = "Reds")[3:6])),
                    #values = rev(c(brewer.pal(n = 12, name = "PiYG")[1:6])),
...
ggsave(paste0("Map 0-5-Deaths.pdf"),height = 12,width = 10,dpi = 500)
